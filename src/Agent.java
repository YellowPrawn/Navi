...
public class Agent {
	...
	Agent(){
		...
		for(int i=0;i<main.grid[0];i++) {//placing walls of window as main.obstacles
			Integer[] wall = {i,-1};
			main.obstacles.add(wall);
			Integer[] wall2 = {i,main.grid[1]};
			main.obstacles.add(wall2);
		}
		for(int i=0;i<main.grid[0];i++) {//placing walls of window as main.obstacles
			Integer[] wall = {-1,i};
			main.obstacles.add(wall);
			Integer[] wall2 = {main.grid[0],i};
			main.obstacles.add(wall2);
		}
	}
		...
		for(int i = 0;i<input.length;i++) {//repeats until all inputs are checked
			...
			for(int j = 0;j<(main.grid[0]+main.grid[1]);j++) {//repeats until vision reaches end of screen
				...
				for(int k = 0;k<main.obstacles.size();k++) {//repeats until all obstacles are checked
					if((temp[0]==main.obstacles.get(k)[0])&&(temp[1]==main.obstacles.get(k)[1])) {//checks if vision position is an obstacle
